...
$mdFiles = [];
$files = (array) glob(rex_addon::get('structure_metainfo')->getPath('docs') . '/*.md');
...
if (null !== $page) {
    foreach ($mdFiles as $key => $mdFile) {
        ...
        $page->addSubpage(
            (new rex_be_page($key, rex_i18n::msg('structure_metainfo_docs_' . $keyWithoudPrio)))
...
echo rex_view::title($this->i18n('structure_metainfo_title'));
